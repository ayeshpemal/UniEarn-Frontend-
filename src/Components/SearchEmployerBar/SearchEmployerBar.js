...
const searchEmployers = async (searchTerm, userId, page = 0) => {
  try {
    const response = await axios.get(
      `${baseUrl}/api/employers/search/follow?userId=${userId}&query=${encodeURIComponent(searchTerm)}&page=${page}`,
      {
        headers: {
...
const EmployerSearchBar = ({ onSearchResults, currentPage = 0 }) => {
  ...
  const fetchInitialResults = async () => {
    console.log("Fetching initial employer results with userId:", currentUserId);
    const results = await searchEmployers("", currentUserId, 0);
  ...
  const performSearch = async (term, page) => {
    ...
    const results = await searchEmployers(term, currentUserId, page);
    if (results && onSearchResults) {
